...
#pragma -mark  解析表情图片
    ...
    if([self.images count]>0){
        ...
        NSInteger imgLocation =[[nextImage objectForKey:MatchParserLocation] integerValue];
        ...
        for (id lineObj in lines) { //5
            ...
            for (id runObj in (__bridge NSArray *)CTLineGetGlyphRuns(line1)) { //6
                ...
                if ( runRange.location==imgLocation) { //7
                    ...
                    [dic setObject:[NSNumber numberWithInteger:lineIndex] forKey:MatchParserLine];
                    //load the next image //12
                    
                    imgIndex++;
                    if (imgIndex < [self.images count]) {
                        nextImage = [self.images objectAtIndex: imgIndex];
                        imgLocation =[[nextImage objectForKey:MatchParserLocation] integerValue];
                    }else{
...
-(void)addLink:(NSRange)range string:(NSString*)string linkType:(NSString*)linkType   link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:string,MatchParserString,[NSValue valueWithRange:range],MatchParserRange,[[NSMutableArray alloc]init],MatchParserRects,linkType,MatchParserLinkType,nil];
    [_links addObject:dic];
    [_links sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        NSDictionary * dic1=obj1;
        NSDictionary * dic2=obj2;
